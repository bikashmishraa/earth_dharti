...
function checkSatelliteHover() {
  ...
  if (intersects.length > 0) {
    ...
    if (index !== -1) {
      ...
      document.getElementById("satelliteName").textContent =
        tleLines[0].slice(1) || "N/A";
      document.getElementById("tleLine1").textContent =
        tleLines[1].slice(1) || "N/A";
      document.getElementById("tleLine2").textContent =
        tleLines[2].slice(1) || "N/A";
    }
  } else {
    tooltip.style.display = "none";
...
function zoomToSatellite(satelliteSprite) {
  ...
  const targetPosition = satelliteSprite.position
    .clone()
    .multiplyScalar(followDistance); // Zoom distance
...
function loadSatellitePathForClickedSatellite(satrec, maxPoints = 100) {
  ...
  for (let t = 0; t < maxPoints; t++) {
    ...
    if (positionEci) {
      const positionGd = satellite.eciToGeodetic(positionEci, gmst);
      const longitude =
        satellite.degreesLong(positionGd.longitude) * (Math.PI / 180);
      const latitude =
        satellite.degreesLat(positionGd.latitude) * (Math.PI / 180);
      const altitude = positionGd.height;
...
function followSatellite() {
  if (targetSatellite && !zooming) {
    const followPosition = targetSatellite.position
      .clone()
      .multiplyScalar(followDistance); // Keep a distance

    // Calculate the distance between the camera and the satellite
    const distanceToSatellite = camera.position.distanceTo(
